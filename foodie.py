...
class Bag:
	def __init__(self, bag_type, bag_cap, bag_contents_size):
		self.bag_type = bag_type
		self.bag_cap = bag_cap
		self.bag_contents_size = bag_contents_size
		self.items = []

	'''
	def can_add_item(self, item):
		if (self.bag_contents_size + item.size) <= self.bag_cap:
	...
	def cap_check(self, item):
		if (self.bag_contents_size + item.size) <= self.bag_cap:
				return True
		return False

	def add_item(self, item):
		if self.cap_check(item):
...
class DeliverySystem:
	...
	def bag_items(self, items, robot_num):
		frag_queue = []
		med_queue = []
		small_queue = []
		overflow_list = []
		for item in items:
			if item.freeze:
				if (item.size + self.robots[robot_num].freezerBag.bag_contents_size) <= self.robots[robot_num].freezerBag.bag_cap:
					self.robots[robot_num].freezerBag.add_item(item)
				else:
					overflow_list.append(item)
			
			elif item.fragile:
				frag_queue.append(item)

			elif item.size == 3:
				if (item.size + self.robots[robot_num].paperBag.bag_contents_size) <= self.robots[robot_num].paperBag.bag_cap:
					self.robots[robot_num].paperBag.add_item(item)
				else:
					overflow_list.append(item)
				...
				small_queue.append(item)
					
		for item in med_queue:
			if self.robots[robot_num].paperBag.add_item(item) == False:
				overflow_list.append(item)

			#if (item.size + self.robots[robot_num].paperBag.bag_contents_size) <= self.robots[robot_num].paperBag.bag_cap:
			#	self.robots[robot_num].paperBag.add_item(item)

		for item in small_queue:
			if self.robots[robot_num].paperBag.add_item(item) == False:
				overflow_list.append(item)
